...
# -*- coding: utf-8 -*-
"""
부호 층 복사 자동화  —  APT_PIT 층 뿌리기 v1.1
...
동작 순서:
  ...
  7) 각 목표층 라벨에 대해 반복:
       ...
       ※ Enter는 누르지 않음 — 이 프로그램에서 Enter는 새 부호 생성으로 인식됨
...
def run_floor_spread(labels, start_display, delay_ms, stop_fn):
    ...
    # ⑦ 각 층 라벨 반복 (Enter 없이 붙여넣기 후 Down으로만 이동)
    for label in labels:
        if stop_fn(): return
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("부호 층 복사 자동화  —  APT_PIT 층 뿌리기  v1.1")
    ...
    def _build(self):
        ...
        tk.Label(h, text="APT_PIT 층 뿌리기  v1.1", bg=PANEL, fg=SUB,
